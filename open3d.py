#This code is copy from https://github.com/doubleZ0108/GeoMVSNet
import torch
import numpy as np
import sys
import argparse
...
def write_ply(file, points):
    ...
    # 
    # 
    # pcd = o3d.geometry.PointCloud() #open3d version bug
...
def load_data(root_path, depth_path, scene_name, thresh, device):
...
def open3d_filter(data_list, root_path, depth_path, ply_path, prob_thresh, dist_thresh, num_consist):
    with torch.no_grad():
        mkdir_p(ply_path)
        ...
        for i, scene in enumerate(all_scenes):
            ...
            depths, projs, rgbs = load_data(root_path, depth_path, scene, prob_thresh, device='cuda')
            ...
            ply_id = int(scene[4:])
            write_ply('{}/mvsnet{:03d}_l3.ply'.format(ply_path, ply_id), np.concatenate(points, axis=0))
            del points, depths, rgbs, projs

            gc.collect()

            print('Save {}/mvsnet{:03d}.ply successful.'.format(ply_path, ply_id))
