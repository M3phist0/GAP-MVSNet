...
#     def __init__(self, datapath, normalpath, listfile, mode, nviews, ndepths=192, interval_scale=1.06, **kwargs):
...
#                 normal_path = os.path.join(self.normalpath, '{}/{:0>6}_normal.npy'.format(
#                     scan, vid))
...
# s_h, s_w = 0, 0
class MVSDataset(Dataset):
    def __init__(self, datapath, normal_path,  listfile, mode, nviews, ndepths=192, interval_scale=1.06, use_normals=True, **kwargs):
        super(MVSDataset, self).__init__()
        self.datapath = datapath
        self.listfile = listfile
        self.mode = mode
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:
                normal_path =  os.path.join(self.normal_path , '{}/{:0>6}_normal.npy'.format(
                    scan, vid))
                normal = self.read_normal(normal_path)
                if not self.fix_wh:
                    # using the same standard height or width in each nviews.
            ...
            if i == 0:  # reference view
                ...
                # print(depth_min, depth_interval * (self.ndepths) + depth_min, depth_interval)
