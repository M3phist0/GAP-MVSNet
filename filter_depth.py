...
def filter_depth(scan_folder, out_folder, plyfilename, geo_pixel_thres, geo_depth_thres, photo_thres, img_wh, geo_mask_thres=3):
    ...
    for ref_view, src_views in pair_data:
        ...
        # if ref_depth_est.shape[0] != img_wh[0] or ref_depth_est.shape[1] != img_wh[1]:
        #     ref_depth_est = cv2.resize(ref_depth_est, (img_wh[0], img_wh[1]), interpolation=cv2.INTER_LINEAR)
        #     confidence = cv2.resize(confidence, (img_wh[0], img_wh[1]), interpolation=cv2.INTER_LINEAR)
        ...
        for src_view in src_views:
            ...
            # if src_depth_est.shape[0] != img_wh[0] or src_depth_est.shape[1] != img_wh[1]:
            #     src_depth_est = cv2.resize(src_depth_est, (img_wh[0], img_wh[1]), interpolation=cv2.INTER_LINEAR)
...
if __name__ == '__main__':
    ...
    if args.dataset=="dtu_yao_eval":
        ...
        if args.split == "test":
            ...
            scans = ['observatory', 'old_computer', 'statue', 'terrace_2']
