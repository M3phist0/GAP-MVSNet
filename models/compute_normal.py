...
def depth2normal(depth_torch, intrinsic_torch, kernel_size=5, dilation=2):
    ...
    point_matrix = F.unfold(points, kernel_size=kernel_size, stride=1, padding=padding, dilation=dilation)

    # An = b
    matrix_a = point_matrix.view(B, 3, kernel_size ** 2, H, W)  # (B, 3, 25, HxW)
    matrix_a = matrix_a.permute(0, 3, 4, 2, 1) # (B, HxW, 25, 3)
    matrix_a_trans = matrix_a.transpose(3, 4)
    matrix_b = torch.ones([B, H, W, kernel_size ** 2, 1], device=depth_torch.device)
